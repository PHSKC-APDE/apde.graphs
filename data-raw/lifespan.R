...
# Date: 2024-12-20
# R version: 4.4.1
# Purpose: Create synthetic data of of life span in ancient cities for ggplot samples
...
# Create Raw / line level data ----
  ...
  # Create the synthetic raw data table
  dt_raw <- data.table(
    city = rep(cities, each = n_per_city),
    lifespan = unlist(lapply(mean_lifespans, function(mean) round(rnorm(n_per_city, mean = mean, sd = 5), 0)))
  )
